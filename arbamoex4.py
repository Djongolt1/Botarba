...
def check_and_create_lock():
    """Проверяет, не запущен ли уже бот, и создаёт блокировку с PID."""
    if os.path.exists(LOCK_FILE):
        try:
            ...
            try:
                ...
                # Процесс мёртв, можно перезаписать блокировку
...
# Вызываем проверку до импорта остальных модулей (чтобы не запускать дважды)
...
DAYS_BACK = 14                     # увеличен до 14 дней
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    try:
        ...
        try:
            ...
            logger.warning(f"Не удалось загрузить 15min: {e}. Пробуем дневные.")
            data1 = t1.candles(start=start_str, end=end_str, period='1d')
            data2 = t2.candles(start=start_str, end=end_str, period='1d')
            intraday = False
...
# ======= ЗАПУСК =======
async def main():
    # Принудительно удаляем вебхук, чтобы исключить конфликт
    ...
    try:
        ...
        # Удаляем файл блокировки при корректном завершении
        if os.path.exists(LOCK_FILE):
            os.remove(LOCK_FILE)
