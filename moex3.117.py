...
def fetch_candles(ticker, start_date, end_date, interval, timeout=30, retries=0):
    url = f"https://iss.moex.com/iss/engines/stock/markets/shares/boards/tqbr/securities/{ticker}/candles.json"
    params = {
        'from': start_date.strftime('%Y-%m-%d'),
        'till': end_date.strftime('%Y-%m-%d'),
        'interval': interval,  # теперь 240 для 4-часовых
        'limit': 10000
    }
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    }
    logger.info(f"Запрос к MOEX: {ticker}, период {start_date} – {end_date}, интервал {interval} мин")
    for attempt in range(retries + 1):
        try:
            ...
            # Принудительная обрезка по датам
            mask = (df.index >= pd.Timestamp(start_date)) & (df.index <= pd.Timestamp(end_date))
            df = df.loc[mask]
            if df.empty:
                logger.warning(f"После обрезки по датам данных для {ticker} нет")
                return None
            logger.info(f"Загружено {len(df)} свечей для {ticker} (интервал {interval} мин)")
            return df
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    # 1) 4-часовые свечи (интервал 240 минут)
    df1 = fetch_candles(ticker1, start_date, end_date, 240, timeout=30, retries=0)
    df2 = fetch_candles(ticker2, start_date, end_date, 240, timeout=30, retries=0)
    if df1 is None or df2 is None or df1.empty or df2.empty:
        logger.warning("4-часовые не загружены, пробуем дневные")
        # 2) Дневные свечи (интервал 1440 минут = 1 день)
        df1 = fetch_candles(ticker1, start_date, end_date, 1440, timeout=45, retries=0)
        df2 = fetch_candles(ticker2, start_date, end_date, 1440, timeout=45, retries=0)
        if df1 is None or df2 is None or df1.empty or df2.empty:
            return None
    # Объединяем
    ...
    if combined.empty:
        ...
        return None
    # Для внутридневных оставляем часы торгов (актуально для 4-часовых)
...
# ======= ЗАПУСК =======
async def main():
    request = HTTPXRequest(read_timeout=90, write_timeout=90, connect_timeout=90, pool_timeout=90)
