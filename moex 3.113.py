...
DAYS_BACK = 5  # теперь везде 5 дней
...
def fetch_candles(ticker, start_date, end_date, interval, timeout=60, retries=2):
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    # 1) 15-минутки
    df1 = fetch_candles(ticker1, start_date, end_date, 15, timeout=120, retries=3)
    df2 = fetch_candles(ticker2, start_date, end_date, 15, timeout=120, retries=3)
    if df1 is None or df2 is None or df1.empty or df2.empty:
        logger.warning("15-минутки не загружены, пробуем часовые")
        # 2) Часовые
        df1 = fetch_candles(ticker1, start_date, end_date, 60, timeout=90, retries=2)
        df2 = fetch_candles(ticker2, start_date, end_date, 60, timeout=90, retries=2)
        if df1 is None or df2 is None or df1.empty or df2.empty:
            logger.warning("Часовые не загружены, пробуем дневные")
            # 3) Дневные (тоже за 5 дней)
