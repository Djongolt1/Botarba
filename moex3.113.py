...
# ======= ФУНКЦИЯ ЗАГРУЗКИ СВЕЧЕЙ (с повторными попытками) =======
def fetch_candles(ticker, start_date, end_date, interval='15', retries=3):
    ...
    for attempt in range(retries + 1):
        try:
            resp = requests.get(url, params=params, timeout=60)  # увеличен таймаут
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    # Пытаемся сначала загрузить 15-минутки
    ...
    if combined.empty:
        ...
        return None
    # Оставляем часы торгов (для внутридневных)
    if len(combined) > 0 and combined.index[0].hour is not None:
...
def generate_spread_plot(pair_name):
    ...
    ax.set_title(f'{pair_name} — спред ({ticker1} – {ticker2}) за {DAYS_BACK} дней', fontsize=14)
