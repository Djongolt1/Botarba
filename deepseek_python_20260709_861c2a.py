...
# ======= ФУНКЦИЯ ЗАГРУЗКИ СПРЕДА (только дневные свечи) =======
def get_spread_data(ticker1, ticker2, start_date, end_date):
    """Загружает дневные свечи через ISS API MOEX и возвращает DataFrame со спредом."""
    try:
        # Для дневных свечей используем interval=1440 (минуты в сутках)
        params = {
            'from': start_date.strftime('%Y-%m-%d'),
            'till': end_date.strftime('%Y-%m-%d'),
            'interval': 1440,  # дневные свечи
            'iss.json': 'extended',
        }
        logger.info(f"Запрашиваем данные для {ticker1} и {ticker2} с {start_date} по {end_date}")
        ...
        def fetch_candles(ticker):
            ...
            resp = session.get(url, params=params, timeout=60)  # увеличен таймаут
        ...
        df1 = fetch_candles(ticker1)
        df2 = fetch_candles(ticker2)
