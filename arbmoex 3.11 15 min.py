...
DAYS_BACK = 2               # уменьшено до 2 дней, чтобы ускорить загрузку
CANDLE_INTERVAL = 15        # 15-минутные свечи
...
def get_http_session():
    ...
    # Уменьшаем таймауты и количество повторных попыток
...
def fetch_candles(ticker, start_date, end_date, interval=CANDLE_INTERVAL):
    ...
    for board in boards:
        ...
        try:
            resp = session.get(url, params=params, timeout=30)  # уменьшен таймаут до 30 сек
            ...
            # --- Попытка распарсить в формате списка с объектами ---
