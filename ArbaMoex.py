...
# ======= НАСТРОЙКИ =======
TELEGRAM_TOKEN = "8663503148:AAEr1Vss6v1vLHf47u1M-Vp_wbhIn0PLBd8"  # замените на свой токен
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    try:
        t1 = Ticker(ticker1)
        t2 = Ticker(ticker2)
        # Приводим даты к строке YYYY-MM-DD
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Отправляем постоянную клавиатуру, если её ещё нет
...
async def handle_main_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Старт":
        ...
        # Неизвестный текст – просто показываем клавиатуру
...
async def main():
    ...
    while True:  # бесконечный цикл перезапуска при сбоях
        try:
            ...
            logger.exception(f"Неожиданная ошибка: {e}. Перезапуск через 10 секунд...")
            await asyncio.sleep(10)
        finally:
    ...
    # Снятие блокировки при окончательном выходе
    if os.name == "nt":
