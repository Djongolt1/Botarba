...
# ======= ФУНКЦИЯ ЗАГРУЗКИ ДНЕВНЫХ СВЕЧЕЙ =======
def fetch_daily_candles(ticker, start_date, end_date):
    """
    Загружает дневные свечи (интервал 1) и обрезает по датам.
    """
    ...
    logger.info(f"Запрос дневных свечей для {ticker} с {start_date} по {end_date}")
    try:
        ...
        # Обрезаем по датам (на всякий случай)
        mask = (df.index >= pd.Timestamp(start_date)) & (df.index <= pd.Timestamp(end_date))
        df = df.loc[mask]
        if df.empty:
            logger.warning(f"После обрезки данных для {ticker} нет")
            return None
        logger.info(f"Загружено {len(df)} дневных свечей для {ticker}")
        return df
    except Exception as e:
        logger.error(f"Ошибка загрузки для {ticker}: {e}")
        return None

def get_spread_data(ticker1, ticker2, start_date, end_date):
    df1 = fetch_daily_candles(ticker1, start_date, end_date)
    df2 = fetch_daily_candles(ticker2, start_date, end_date)
...
def generate_spread_plot(pair_name):
    ...
    xticks_labels = [data.index[i].strftime("%Y-%m-%d") for i in xticks_pos]
