...
# ======= НАСТРОЙКИ =======
TELEGRAM_TOKEN = "8885452009:AAGxNl0iUCT2Q58jXm0DZ_h-ZDevQcYtqpw"  # новый токен
CHAT_ID = 1234329121
UPDATE_INTERVAL_MINUTES = 60
DAYS_BACK = 14  # увеличен до 14 дней (максимум)
...
# ======= ФУНКЦИЯ ЗАГРУЗКИ СПРЕДА (улучшенная) =======
def get_spread_data(ticker1, ticker2, start_date, end_date):
    try:
        ...
        try:
            ...
            logger.warning(f"15min не доступны ({e}), берём дневные свечи для {ticker1}/{ticker2}")
            data1 = t1.candles(start=start_str, end=end_str)
            data2 = t2.candles(start=start_str, end=end_str)
            intraday = False

        if not data1 or not data2:
            logger.warning(f"Нет данных от MOEX для {ticker1} или {ticker2}")
            return None
        ...
        logger.info(f"Получено {len(df1)} свечей для {ticker1}, {len(df2)} для {ticker2}")

        if df1.empty or df2.empty:
            logger.warning(f"Пустые DataFrame для {ticker1} или {ticker2}")
            return None
        ...
        if intraday:
            # Оставляем только часы торгов (с 7 утра по Москве)
            combined = combined[combined.index.hour >= 7]

        if combined.empty:
            logger.warning("После объединения данных нет общих временных меток")
            return None
...
def generate_spread_plot(pair_name):
    ...
    now = datetime.now()
    end_date = now.date() + timedelta(days=1)   # запрос до завтра включительно
