...
def fetch_candles(ticker, start_date, end_date, interval, timeout=45, retries=1):
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    ...
    if df1 is None or df2 is None or df1.empty or df2.empty:
        ...
        # 2) Дневные свечи (интервал 1)
        df1 = fetch_candles(ticker1, start_date, end_date, 1, timeout=60, retries=2)
        df2 = fetch_candles(ticker2, start_date, end_date, 1, timeout=60, retries=2)
        if df1 is None or df2 is None or df1.empty or df2.empty:
            return None
    # Объединяем
    ...
    if combined.empty:
        ...
        return None
    # Для внутридневных оставляем часы торгов (для часовых это актуально)
...
# ======= ЗАПУСК =======
async def main():
    request = HTTPXRequest(read_timeout=120, write_timeout=120, connect_timeout=120, pool_timeout=120)
