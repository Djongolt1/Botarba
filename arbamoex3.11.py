...
DAYS_BACK = 7               # для часовых свечей достаточно 7 дней
CANDLE_INTERVAL = 60        # 60 минут = часовые свечи. Можно сменить на 1440 (дневные) или 15 (15-минутные)
...
def fetch_candles(ticker, start_date, end_date, interval=CANDLE_INTERVAL):
    ...
    for board in boards:
        ...
        try:
            ...
            # Логируем первые 300 символов для отладки (при необходимости)
            # logger.debug(f"Ответ MOEX для {ticker} (доска {board}): {json.dumps(data, indent=2, ensure_ascii=False)[:300]}...")
            ...
            if isinstance(candles_block, list) and len(candles_block) >= 2:
                # Второй элемент обычно содержит массив объектов с данными
                for item in candles_block:
                    if isinstance(item, list) and len(item) > 0 and isinstance(item[0], dict):
                        rows = item
                        # Проверяем наличие ключа 'begin' в первом объекте
                        if 'begin' in rows[0]:
