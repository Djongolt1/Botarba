...
DAYS_BACK = 7  # для 15-минуток и часовых
DAYS_BACK_DAILY = 30  # для дневных свечей
...
def fetch_candles(ticker, start_date, end_date, interval='15', timeout=30, retries=1):
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    # 1) Пробуем 15-минутки
    df1 = fetch_candles(ticker1, start_date, end_date, '15', timeout=30, retries=1)
    df2 = fetch_candles(ticker2, start_date, end_date, '15', timeout=30, retries=1)
    if df1 is None or df2 is None or df1.empty or df2.empty:
        logger.warning("15-минутки не загружены, пробуем часовые")
        # 2) Часовые
        df1 = fetch_candles(ticker1, start_date, end_date, '60', timeout=45, retries=1)
        df2 = fetch_candles(ticker2, start_date, end_date, '60', timeout=45, retries=1)
        if df1 is None or df2 is None or df1.empty or df2.empty:
            logger.warning("Часовые не загружены, пробуем дневные за последние 30 дней")
            # 3) Дневные с расширенным периодом (30 дней)
            end_date_daily = end_date
            start_date_daily = end_date_daily - timedelta(days=DAYS_BACK_DAILY)
            df1 = fetch_candles(ticker1, start_date_daily, end_date_daily, '1', timeout=60, retries=2)
            df2 = fetch_candles(ticker2, start_date_daily, end_date_daily, '1', timeout=60, retries=2)
            if df1 is None or df2 is None or df1.empty or df2.empty:
                return None
    # Объединяем
