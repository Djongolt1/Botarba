...
# ======= ФУНКЦИЯ ЗАГРУЗКИ СВЕЧЕЙ (без обрезки) =======
def fetch_candles(ticker, start_date, end_date, interval='15', retries=2):
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    ...
    if df1 is None or df2 is None or df1.empty or df2.empty:
        ...
        df1 = fetch_candles(ticker1, start_date, end_date, '60')
        df2 = fetch_candles(ticker2, start_date, end_date, '60')
        if df1 is None or df2 is None or df1.empty or df2.empty:
    ...
    if combined.empty:
        ...
        return None
    # Для внутридневных оставляем часы торгов (если есть hour)
