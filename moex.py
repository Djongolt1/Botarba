...
# ======= НАСТРОЙКИ =======
TELEGRAM_TOKEN = "8885452009:AAGQ2c2qTZkqrLm_E66zehioGkG53D5lrls"  # новый токен
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    ...
    try:
        ...
        if df1 is None or df2 is None or df1.empty or df2.empty:
            logger.warning(f"Нет данных для {ticker1} или {ticker2}")
            return None
...
def generate_spread_plot(pair_name):
    ...
    # Настройка подписей оси X (даты)
    step = max(1, len(data) // 10)  # для дневных данных показываем меньше меток
