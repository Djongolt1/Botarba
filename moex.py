...
DAYS_BACK = 7
# ИЗМЕНЕНО: интервал свечей теперь 30 минут
CANDLE_INTERVAL = 30        # 30-минутные свечи
...
# ======= ДОБАВЛЕНО: получение последней цены по тикеру =======
def get_last_price(ticker):
    """Возвращает последнюю цену акции через MOEX marketdata."""
...
def get_current_spread(pair_name):
    """Возвращает (цена1, цена2, спред) для пары по последним ценам, или None."""
...
def fetch_candles(ticker, start_date, end_date, interval=CANDLE_INTERVAL):
    ...
    Автоматически определяет формат ответа и возвращает DataFrame с индексом 'begin' и колонкой 'close'.
    """
    params = {
...
def generate_spread_plot(pair_name):
    ...
    # Последний спред из свечей (запасной)
    last_spread = y[-1]
...
# ======= ОТПРАВКА ГРАФИКА (ДОБАВЛЕН ТЕКУЩИЙ СПРЕД) =======
async def send_spread_update(chat_id, pair_name, bot):
    try:
        ...
        # Получаем текущий спред по последним ценам
        current = get_current_spread(pair_name)
        caption = f"📊 Спред {pair_name} ({PAIRS[pair_name][0]} – {PAIRS[pair_name][1]}) за последние {DAYS_BACK} дней (30-минутные свечи)"
        if current:
            p1, p2, spread = current
            caption += f"\n\n🟢 Текущий спред (по последним ценам): **{spread:.2f} ₽**\n{PAIRS[pair_name][0]}: {p1:.2f}  |  {PAIRS[pair_name][1]}: {p2:.2f}"
        else:
            caption += "\n\n⚠️ Не удалось получить текущие цены, показан спред по свечам."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Данные дополняются актуальным спредом по последним ценам.\n\n"
