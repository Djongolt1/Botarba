...
def fetch_candles(ticker, start_date, end_date, interval, timeout=60, retries=2, split_days=False):
    """
    Загружает свечи. Если interval=15 и split_days=True, разбивает запрос по дням.
    """
    ...
    # Если не нужно разбивать по дням или интервал не 15, используем обычный запрос
    if not split_days or interval != 15:
        return _fetch_single(ticker, start_date, end_date, interval, timeout, retries, headers)

    # Разбиваем на отдельные дни (только будние, но мы запросим все, MOEX вернёт пусто для выходных)
    current = start_date
    all_dfs = []
    while current <= end_date:
        day_start = current
        day_end = current + timedelta(days=1) - timedelta(seconds=1)  # конец дня
        # Запрашиваем один день с таймаутом 30 сек, 1 попытка
        df_day = _fetch_single(ticker, day_start, day_end, interval, timeout=30, retries=1, headers=headers)
        if df_day is not None and not df_day.empty:
            all_dfs.append(df_day)
        current += timedelta(days=1)
        time.sleep(0.5)  # пауза между запросами, чтобы не перегружать сервер

    if not all_dfs:
        return None
    # Объединяем все DataFrame
    combined = pd.concat(all_dfs).sort_index()
    logger.info(f"Загружено {len(combined)} свечей для {ticker} (интервал {interval}, разбивка по дням)")
    return combined

def _fetch_single(ticker, start_date, end_date, interval, timeout, retries, headers):
    ...
    logger.info(f"Запрос к MOEX: {ticker}, период {start_date} – {end_date}, интервал {interval}")
...
def get_spread_data(ticker1, ticker2, start_date, end_date):
    # 1) 15-минутки с разбивкой по дням (если не загружаются за весь период)
    df1 = fetch_candles(ticker1, start_date, end_date, 15, timeout=120, retries=2, split_days=True)
    df2 = fetch_candles(ticker2, start_date, end_date, 15, timeout=120, retries=2, split_days=True)
    if df1 is None or df2 is None or df1.empty or df2.empty:
        logger.warning("15-минутки не загружены, пробуем часовые")
        # 2) Часовые (без разбивки)
        df1 = fetch_candles(ticker1, start_date, end_date, 60, timeout=90, retries=2, split_days=False)
        df2 = fetch_candles(ticker2, start_date, end_date, 60, timeout=90, retries=2, split_days=False)
        if df1 is None or df2 is None or df1.empty or df2.empty:
            logger.warning("Часовые не загружены, пробуем дневные")
            # 3) Дневные
            df1 = fetch_candles(ticker1, start_date, end_date, 1, timeout=60, retries=2, split_days=False)
            df2 = fetch_candles(ticker2, start_date, end_date, 1, timeout=60, retries=2, split_days=False)
            if df1 is None or df2 is None or df1.empty or df2.empty:
                return None
    # Объединяем
...
# ======= ЗАПУСК =======
async def main():
    request = HTTPXRequest(read_timeout=180, write_timeout=180, connect_timeout=180, pool_timeout=180)
